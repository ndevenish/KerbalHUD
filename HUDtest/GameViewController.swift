...
class GameViewController: GLKViewController {
  ...
  func setupGL() {
    ...
    glBufferData(GLenum(GL_ARRAY_BUFFER), GLsizeiptr(sizeof(GLfloat) * gSquareVertexData.count), &gSquareVertexData, GLenum(GL_STATIC_DRAW))
...
//    glVertexAttribPointer(GLuint(GLKVertexAttrib.Normal.rawValue), 3, GLenum(GL_FLOAT), GLboolean(GL_FALSE), 24, BUFFER_OFFSET(12))
  ...
  func update() {
    ...
    if aspect > 1 {
      projectionMatrix = GLKMatrix4MakeOrtho(-aspect, aspect, -1, 1, -10, 10)
    } else {
      projectionMatrix = GLKMatrix4MakeOrtho(-1, 1, -1.0/aspect, 1.0/aspect, -10, 10)
    }

//    let orthMatrix = GLKMatrix4MakeOrtho(0, 1, 0, 1, -1, 1)
    
//    self.effect?.transform.projectionMatrix = projectionMatrix
    
    var baseModelViewMatrix = GLKMatrix4Identity;
    // GLKMatrix4MakeTranslation(0.0, 0.0, -4.0)
